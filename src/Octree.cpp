...
ngl::Vec4 Octree::getPointsInsideSphere(ngl::Vec3 centre, float radius)
{
  ...
  int inc;
  ngl::Vec4 tempResults(0.0,0.0,0.0);
  ngl::Vec4 temp;
  if(isLeaf())
  {
  ...
  else
  {
    if(m_inc==0)
      for(int i=0;i<8;++i)
      {
        ...
        ngl::Vec3 max;
        max.m_x = m_children[i].m_origin.m_x + m_children[i].m_halfD;
        max.m_y= m_children[i].m_origin.m_y + m_children[i].m_halfD;
        max.m_z=m_children[i].m_origin.m_z + m_children[i].m_halfD;

        ngl::Vec3 min;
        min.m_x = m_children[i].m_origin.m_x - m_children[i].m_halfD;
        min.m_y = m_children[i].m_origin.m_y - m_children[i].m_halfD;
        min.m_z = m_children[i].m_origin.m_z - m_children[i].m_halfD;
        ...
        if(BoidMath::boxIntersectsSphere(min, max, centre, radius))
        {
          ...
          for(int j=0;j<inc;++j)
          {
            ...
            if(temp.m_x!=0.0&&temp.m_y!=0.0&&temp.m_z!=0.0)
            {
...
void Octree::buildVAO()
{
  ...
  else
  {
    ...
    m_boidVAO= ngl::VAOFactory::createVAO(ngl::simpleVAO,GL_LINES);
    m_boidVAO->bind();

    // in this case we are going to set our data as the vertices above

    m_boidVAO->setData(ngl::SimpleVAO::VertexData(verts.size()*sizeof(ngl::Vec3),verts[0].m_x,GL_STATIC_DRAW));
